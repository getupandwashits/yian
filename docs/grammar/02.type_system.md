...
4. **泛型类型**: 在泛型上下文中使用的类型, 本质上是类型占位符, 详见[泛型](05.generics.md)章节
...
- 可以定义并包含类型参数, 使其成为泛型类型, 详见[泛型](05.generics.md)章节
- 可以定义访问修饰符, 例如 `pub`, 详见[模块](06.modules.md)章节
...
如果用户需要自定义类型转换, 可以通过实现标准库中的 `From`, `Into` trait 来实现, 详见[标准库](08.standard_library.md)章节
