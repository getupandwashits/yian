...
- `<lvalue>`: 赋值语句左侧的表达式, 必须是一个可赋值的左值表达式, 关于左值的详细信息请参见[表达式](0X.expressions.md)章节
...
- `<method_name>`: 方法名称
- `<type_arguments>`: 可选的类型参数列表, 仅在调用泛型函数或方法时使用, 详见[泛型](0X.generics.md)章节
...
- 使用这种语法构造结构体实例时, 等效于为结构体的每个字段进行赋值, 会受到访问控制的限制, 详见[模块](0X.modules.md)章节
